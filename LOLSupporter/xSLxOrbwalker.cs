    ...
    public class xSLxOrbwalker
    {
        ...
        public static void Orbwalk(Vector3 goalPosition, Obj_AI_Base target)
        {
            ...
            if (!CanMove() || !IsAllowedToMove())
                return;
            if (MyHero.IsMelee() && target != null && Geometry.Distance(target, MyHero) < GetAutoAttackRange(MyHero, target) &&
        ...
        public static double GetAzirAASandwarriorDamage(Obj_AI_Base unit)
        {
            ...
            Geometry.Distance(obj, unit) < 350) == 2)
        ...
        {
            return target != null && ObjectManager.Get<Obj_AI_Minion>().Any(obj => obj.Name == "AzirSoldier" && obj.IsAlly && obj.BoundingRadius < 66 && obj.AttackSpeedMod > 1 && Geometry.Distance(obj, target) < 380);
        }
        public static Obj_AI_Base GetPossibleTarget()
        {
            ...
            if (CurrentMode == Mode.Harass || CurrentMode == Mode.Lasthit || CurrentMode == Mode.LaneClear || CurrentMode == Mode.LaneFreeze)
            {
                if (MyHero.ChampionName == "Azir")
                {
                    ...
                    1000 * (int)Geometry.Distance(MyHero, minion) / (int)MyProjectileSpeed()
